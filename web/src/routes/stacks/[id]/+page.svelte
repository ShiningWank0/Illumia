<script lang="ts">
  ...
  import { getApi, type Asset, type ChapterInput, type StackDetail } from '$lib/api';
  ...
  let menuFor = $state<string | null>(null); // ページメニューを開いている asset id
  let showDissolve = $state(false);

  // 構造の署名 (順序 + 章タイトルのみ。show_in_timeline は含めない)。
  ...
  // ---- ドラッグ&ドロップ (マウス優先, HTML Drag and Drop API) ----
  let drag = $state<{ ci: number; pi: number } | null>(null);
  let dropTarget = $state<string | null>(null); // "ci:pi" or "ci:end"
...
<svelte:head><title>{stack?.title ?? '漫画スタック'} - Illumia</title></svelte:head>
...
<div class="page">
  ...
  {:else if stack}
    <header>
      <a class="back" href="/stacks">← 一覧</a>
      <input class="title" bind:value={titleDraft} onblur={saveTitle} aria-label="タイトル" />
      <div class="head-actions">
        <a class="btn" href={`/stacks/${stackId}/read?page=1`}>読む</a>
    ...
    <p class="hint muted small">
      ページをドラッグして並べ替え
      (章をまたいで移動可)。ページの「⋯」から話の分割・表紙・表示切替・除外。
    </p>
          ...
          {#each ch.pages as p, pi (p.asset.id)}
            ...
            >
              <div class="thumb" class:trashed={p.asset.status === 'trashed'}>
                <AssetImage id={p.asset.id} thumbhash={p.asset.thumbhash} />
