<script lang="ts">
  ...
  import { getApi, type Asset } from '$lib/api';
  ...
  async function load() {
    ...
    try {
      ...
      pages = flat;
      const requested = Number($page.url.searchParams.get('page') ?? '1');
      current = clamp(
        (Number.isFinite(requested) ? requested : 1) - 1,
        0,
...
<svelte:head><title>{title} - リーダー</title></svelte:head>
<svelte:window onkeydown={onKeydown} />

<div class="reader">
  <header>
    <button class="back" onclick={() => goto(`/stacks/${stackId}`)}>← 戻る (Esc)</button>
      ...
      <div class="prefetch" aria-hidden="true">
        {#each [1, 2] as d (d)}
          {#if pages[current + d] && pages[current + d].asset.status !== 'trashed'}
            <AssetImage id={pages[current + d].asset.id} variant="preview" />
          {/if}
        {/each}
      </div>
    </div>
